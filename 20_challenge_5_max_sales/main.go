...
func matchSales(budget float64, items []SaleItem) []SaleItem {
	//sort.Slice(items,func(i, j int) bool { return items[i].ReducedPrice < items[j].ReducedPrice })
	var mi[]SaleItem
	...
	sort.Slice(mi,func(i, j int) bool { return items[i].SalePercentage > items[j].SalePercentage })
