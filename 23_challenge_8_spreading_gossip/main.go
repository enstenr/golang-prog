...
func spreadGossip(root Friend, friends Friends,gossiped map[string]struct{}) {
	for _,id:=range root.Friends{
		_,hasgossiped:=gossiped[id]
			if !hasgossiped{
				gossiped[id]=struct{}{}
				f:=friends.getFriend(id)
				f.hearGossip()
				spreadGossip(f,friends,gossiped)
...
func main() {
	friends := importData()
	gossiped:=make(map[string]struct{})
	root := friends.getRandomFriend()
	root.hearGossip()
	gossiped[root.ID]=struct{}{}
	spreadGossip(root, friends,gossiped)

	//fmt.Println(" Gossips heard by : ",gossiped)
	fmt.Printf(" Total Friends %v. Total gossip heard : %v",len(friends.fmap),len(gossiped))
	fmt.Println("")
}
